...
TriStateCheckBox.prototype.updateImageState = function(hover)
{
	...
	switch(this.state)
	{
		case TriStateCheckBox.STATE_ALL:
			if(target.hasClass('indeterminate')) target.removeClass('indeterminate');
		...
		case TriStateCheckBox.STATE_SOME:
			if(!target.hasClass('indeterminate')) target.addClass('indeterminate');
			if(target.hasClass('unchecked')) target.removeClass('unchecked');
			if(target.hasClass('checked')) target.removeClass('checked');
			break;
	}
	...
	if(hover)
	{
		...
		if(target.hasClass('hover')) target.removeClass('hover');
	}
};
