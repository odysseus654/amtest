...
(function()
{
	...
	PP.endProgress = function(container)
	{
		if(this.status.length != this.trow.cells.length)
		{
			this.fullUpdateProgress();
		}

		$(this.labelDiv).empty()
			.append($('<div style="float:left"><span class="qcorr"/> of <span class="qtotal"/> Questions (<span class="pcorr"/>%) Correct</div>'));
	...
	LP.endOfTest = function()
	{
		...
		$(this.base).append('<p>No more questions left</p>' +
...
(function()
{
	function onExpandSection(secId)
	{
		...
		if(ourRow.hasClass('collapsed'))
		{
			...
			targets.removeClass('sec-hidden');
		} else {
			ourRow.addClass('collapsed');
			targets.addClass('sec-hidden');
		}
	}
	...
	CL.onExpandSubsection = function(elm, secId, subqlist)
	{
		// handle existing secttions
		var targets = $('TABLE.sec-choice TR.subof-'+secId);
		var ourRow = $(elm).parents('TR');
		if(!ourRow.hasClass('collapsed'))
		{
			ourRow.addClass('collapsed');
			targets.addClass('sub-hidden');
		} else {
			ourRow.removeClass('collapsed');
			targets.removeClass('sub-hidden');
	...
	CL.showCategories = function()
	{
		...
		for(var sec=0; sec < secList.length; sec++)
		{
			...
			for(var sub=0; sub < subList.length; sub++)
			{
				...
				$('SPAN.expander', subRow).on('click', curry(function(secId, subqlist)
				{
					self.onExpandSubsection(this, secId, subqlist);
				}, subObj.name, subqlist));
